...
exports.downloadAttemptPDF = catchAsync(async (req, res, next) => {
    ...
    include: [
      {
        model: PracticeTest,
        as: 'test',
        attributes: ['title', 'duration', 'passingScore', 'category']
      },
      {
        model: User,
        as: 'user',
  ...
  // Only allow PDF download for completed tests
  if (testAttempt.status !== 'completed') {
    return next(new AppError('PDF can only be downloaded for completed tests', 400));
  }
  ...
  try {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="test-result-${testAttemptId}.pdf"`);
    
    // Pipe PDF to response
    doc.pipe(res);

    // Add header
    doc.fontSize(20).text('IIFTL Test Result Report', { align: 'center' });
    doc.moveDown(2);
    ...
    doc.moveDown(0.5);
    doc.fontSize(12);
    doc.text(`Test Title: ${testAttempt.testTitle || 'N/A'}`);
    doc.text(`Category: ${testAttempt.test?.category || 'N/A'}`);
    doc.text(`Duration: ${testAttempt.test?.duration || 'N/A'} minutes`);
    doc.text(`Passing Score: ${testAttempt.test?.passingScore || 'N/A'}%`);
    doc.moveDown(1);
    ...
    doc.moveDown(0.5);
    doc.fontSize(12);
    doc.text(`Name: ${testAttempt.user?.firstName || ''} ${testAttempt.user?.lastName || ''}`);
    doc.text(`Email: ${testAttempt.user?.email || 'N/A'}`);
    doc.text(`Student ID: ${testAttempt.user?.studentId || 'N/A'}`);
    doc.moveDown(1);
    ...
    doc.moveDown(0.5);
    doc.fontSize(12);
    doc.text(`Score: ${testAttempt.score || 0}%`);
    doc.text(`Status: ${testAttempt.passed ? 'PASSED' : 'FAILED'}`);
    doc.text(`Correct Answers: ${testAttempt.correctAnswers || 0}`);
    doc.text(`Wrong Answers: ${testAttempt.wrongAnswers || 0}`);
    doc.text(`Total Questions: ${testAttempt.totalQuestions || 0}`);
    doc.text(`Time Taken: ${testAttempt.timeTaken || 0} seconds`);
    doc.moveDown(1);
    ...
    doc.moveDown(0.5);
    doc.fontSize(12);
    doc.text(`Attempt Date: ${testAttempt.completedAt ? new Date(testAttempt.completedAt).toLocaleString() : 'N/A'}`);
    doc.text(`Attempt Number: ${testAttempt.attemptsCount || 1}`);
    doc.moveDown(1);

    // Questions and answers (if available)
    if (testAttempt.questionsAsked && testAttempt.questionsAsked.length > 0) {
      doc.fontSize(16).text('Questions and Answers', { underline: true });
      doc.moveDown(0.5);
      
      testAttempt.questionsAsked.forEach((question, index) => {
        doc.fontSize(12).text(`Question ${index + 1}:`, { underline: true });
        doc.text(question.question || 'N/A');
        doc.moveDown(0.3);
        ...
        doc.moveDown(0.3);
        doc.text(`Correct Answer: ${question.correctAnswer || 'N/A'}`);
        
        if (testAttempt.answers && testAttempt.answers[index] !== undefined) {
          doc.text(`Your Answer: ${testAttempt.answers[index] || 'No answer'}`);
        }
    ...
    // Footer
    doc.moveDown(2);
    doc.fontSize(10).text('Generated by IIFTL Learning Platform', { align: 'center' });
