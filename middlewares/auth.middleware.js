...
exports.protect = async (req, res, next) => {
  ...
  try {
    ...
    // ✅ FIX: Make session validation mandatory for all users (except when sessionId is not provided)
    if (sessionId) {
      // Validate the session when provided
      ...
      // ✅ FIX: For requests without sessionId, still check for any active sessions and validate them
      // This ensures idle timeout is applied even when sessionId header is missing
      ...
      if (activeSessions.length > 0) {
        // Check if any session is idle or expired
        for (const session of activeSessions) {
          if (session.isIdle(30) || session.isExpired()) {
            await UserSession.update({ isActive: false }, { where: { id: session.id } });
          } else {
            // Update activity for the most recent session
            await session.updateActivity();
            break;
          }
        }
        
        // Check if all sessions are now inactive
        const stillActiveSessions = await UserSession.findAll({
          where: { 
            userId: user.id, 
            isActive: true,
            accessToken: token
        ...
        if (stillActiveSessions.length === 0) {
          return res.status(401).json({
            status: 'fail',
            message: 'Session expired due to inactivity. Please login again.',
            code: 'SESSION_IDLE_TIMEOUT'
